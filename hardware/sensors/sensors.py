...
class VirtualIMU(VirtualSensor):
    ...
    def measure(self, omega_true: np.ndarray):
        """
        ...
        """
        dt = 1.0 / self.rate_hz # time Step between logged points 
        ...
        tau = 300.0 # need to determine
        alpha = np.exp(-dt / tau)
        noise_var = bias_instability_rad_hr ** 2 * (1 - np.exp(-2 * dt / tau))
        ...
        omega_limited = np.clip(omega_adjusted, -self.gyro_lims_rad_s, self.gyro_lims_rad_s)
